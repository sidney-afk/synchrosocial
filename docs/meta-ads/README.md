...
| **`Schedule`** ← QUALIFIED bookings only (CHANGED 2026-08-27) | **server-side only**, via Conversions API | No longer fired from the browser. n8n workflow **"Sales - Qualified Call to Meta CAPI"** (`12ucMkfW2j10sIDj`) receives iClosed's Contact-by-status webhook at `/webhook/iclosed-qualified-capi` and POSTs a standard `Schedule` event to the dataset ONLY when `is_acquisition && hasCall && status === "qualified"`. Deduped durably in the `meta_capi_sent` Data Table (`0PRw0Vtw8eKGUche`), because iClosed re-fires status events and Meta's own event_id dedupe only covers 48h. User data is SHA-256 hashed (email, phone, first, last) plus `fbc`/`fbp` when iClosed captured them. |
